...
""" 
Connecting our redisgraph
"""

def get_person_products(redis_graph, person_id):
    """

    :param redis_graph: Our redisgraph db created before
    :param person_id:
    :return:
    """
    product_query = """MATCH (p:person {id: %s})-[v:buy]->(pr:product)      
               RETURN pr.name""" % person_id
              # Returns with particular id (have given before) and shows bought items
...
def get_similar_persons(redis_graph, person_id, person_limit):
    ...
    :param redis_graph: Redisgraph (case_graph)
    :param person_id: given id starting 0
    :param person_limit: particular limit
    :return:
    """
...
if __name__ == '__main__':
    ...
    if recommended_product is not None:
        ...
        print(f"Recommendation failed! Person with ID = {person_id} have already bought all of products or no similarity data !")
